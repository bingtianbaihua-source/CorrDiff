...
def sample_diffusion_ligand(model, data, num_samples, batch_size=16, device='cuda',
                            num_steps=None, center_pos_mode='protein', sample_num_atoms='prior', objective = ['affinity', 'qed'], w=[1,1,1,1,1]):
...
def main():
    ...
    parser.add_argument('--config', type=str, default='/home/zhoujingyuan/KGDiff-main/configs/sampling.yml')
    ...
    parser.add_argument('--result_path', type=str, default='/data2/zhoujingyuan/MoC/outputs/')
    ...
    ckpt['config'].data.name = 'pl_chem'
    # Load dataset
    dataset, subsets = get_dataset(
        config=ckpt['config'].data,
        transform=transform
    )
    if ckpt['config'].data.name == 'pl' or ckpt['config'].data.name == 'pl_chem':
