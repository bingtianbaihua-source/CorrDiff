...
def main():
    ...
    if args.start is not None and args.end is not None:
        results_fn_list = results_fn_list[args.start : args.end]
    ...
    for example_idx, r_name in enumerate(tqdm(results_fn_list, desc='Eval')):
        ...
        all_pred_exp_score = r['pred_affinity']
        all_pred_exp_atom_traj = r['pred_affinity_atom_traj']
        num_samples += len(all_pred_ligand_pos)
        results_for_one_protein = []
        for sample_idx, (pred_pos, pred_v, pred_exp_score, pred_exp_atom_weight) in enumerate(zip(all_pred_ligand_pos, all_pred_ligand_v, all_pred_exp_score, all_pred_exp_atom_traj)):
            ...
            results_for_one_protein.append({
                'mol': mol,
                'smiles': smiles,
                'ligand_filename': r['data'].ligand_filename,
                'ligand_name': f'res_{sample_idx}.sdf',
                'pred_pos': pred_pos,
                'pred_v': pred_v,
        ...
        results.append(results_for_one_protein)
        
        if example_idx % 10 == 0:
            temp_results = [r for pr in results for r in pr]
            
            qed = [r['chem_results']['qed'] for r in temp_results]
            sa = [r['chem_results']['sa'] for r in temp_results]
            logp = [r['chem_results']['logp'] for r in temp_results]
            lipinski = [r['chem_results']['lipinski'] for r in temp_results]
            ...
            logger.info('LogP:    Ratio: %.3f ' % ((sum(1 for num in logp if -0.4 <= num <= 5.6))/len(logp)))
            logger.info('Lipinski:   Mean: %.3f Median: %.3f' % (np.mean(lipinski), np.median(lipinski)))
            vina_score_only = [r['vina']['score_only'][0]['affinity'] for r in temp_results]
            vina_min = [r['vina']['minimize'][0]['affinity'] for r in temp_results]
            ...
            if args.docking_mode == 'vina_dock':
                vina_dock = [r['vina']['dock'][0]['affinity'] for r in temp_results]
    ...
    logger.info('Number of reconstructed mols: %d, complete mols: %d, evaluated mols: %d' % (
        n_recon_success, n_complete, len(temp_results)))
    ...
    logger.info('Lipinski:   Mean: %.3f Median: %.3f' % (np.mean(lipinski), np.median(lipinski)))
    if args.docking_mode == 'qvina':
        vina = [r['vina'][0]['affinity'] for r in temp_results]
    ...
    ring_info = print_ring_ratio([r['chem_results']['ring_size'] for r in temp_results], logger)
